...
ENV LANG=C.UTF-8
ARG username=chillbilly
ARG user_id=700
RUN adduser -u $user_id -D $username && RUN mkdir /home/${username}
WORKDIR /var/www/app
...
# Set ownership of work dir and home dir to ruby user
RUN chown -R ${username}:${username} /var/www/app && RUN chown -R ${username}:${username} /home/${username}
...
# Anticipate using Puma server
EXPOSE 3000

# Anticipate using Mailcatcher
EXPOSE 1080
